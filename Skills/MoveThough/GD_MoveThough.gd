...
func select() -> void:
	if player.is_move:
		...
		return
	var cood_list = []
	for i in range(-player.move_range,player.move_range):
		for j in range(-player.move_range,player.move_range+1):
			if abs(i)+abs(j) <= player.move_range and abs(i)+abs(j) != 0:
